...
class NodeClassification(BaseTask):
    ...
    def __init__(self, args):
        ...
        # according to the code, build_dataset is the function that generate the data,
        # maybe the logger is the reason why makes it waste a lot of time.
        if hasattr(args, 'sperate_dataset'):
            print("the dataset are build before")
            self.logger.info("the task used the args.sperate_dataset")
            self.dataset = args.sperate_dataset
        else:
            print("the dataset build every epoch")
            self.dataset = build_dataset(args.dataset, 'node_classification', logger=self.logger)
            # it seems that we can load the data in this place
        # openhgnn.dataset.NodeClassificationDataset.HGB_NodeClassification
        # self.evaluator = Evaluator()
        self.logger = args.logger
        # it seems that the rated of the split is 7:2:1
        ...
        if hasattr(args, 'evaluation_metric'):
            ...
            if args.dataset in ['aifb', 'mutag', 'bgs', 'am']:
                ...
                # used the metric micro_f1,macro_f2
                self.evaluation_metric = 'f1'

    def get_graph(self):
        return self.dataset.g

    # here the loss function I need to rewrite.
