...
class Logger:
    ...
    def __init__(self, config):
        ...
        """
        init(autoreset=True)  # init the color of the output
        LOGROOT = f'./openhgnn/output/'  # this line define the path of output.
        # I dont know whether should change the path
        dir_name = os.path.dirname(LOGROOT)  # get the dir name remove the last /
        ensure_dir(dir_name)  # make the dir.

        logfilename = '{}-{}.log'.format(config.Ismetatrain, get_local_time())
    ...
    def log_data_info(self, g):
        ...
        datainfo = {'total nodes': num_nodes, 'total edges': num_edges, 'node types': node_types,
