...
# the hgb_odeClassificationDataset class is herit from this class, so the get_label() is from here.
...
class HIN_NodeClassification(NodeClassificationDataset):
    ...
    def load_HIN(self, name_dataset):
        ...
        if name_dataset == 'demo_graph':
            ...
            g = dataset[0].long()
            num_classes = 4
        # this dataset is very simple,we can used it.
...
# here the class is what we need.
...
class HGB_NodeClassification(NodeClassificationDataset):
    ...
    def __init__(self, dataset_name, *args, **kwargs):
        ...
        if dataset_name == 'HGBn-ACM':
            # here I will write something about the HGBDataset Class
            # 'HGBn-ACM', 'HGBn-DBLP', 'HGBn-Freebase', 'HGBn-IMDB','HGBl-amazon', 'HGBl-LastFM', 'HGBl-PubMed'
            # are in this class
            dataset = HGBDataset(name=dataset_name, raw_dir='')
            g = dataset[0].long()
            """DGLHeteroGraph.long
            Graph(num_nodes={'author': 5959, 'paper': 3025, 'subject': 56, 'term': 1902},
      num_edges={('author', 'author-paper', 'paper'): 9949, ('paper', 'paper-author', 'author'): 9949, ('paper', 'paper-cite-paper', 'paper'): 5343, ('paper', 'paper-ref-paper', 'paper'): 5343, ('paper', 'paper-subject', 'subject'): 3025, ('paper', 'paper-term', 'term'): 255619, ('subject', 'subject-paper', 'paper'): 3025, ('term', 'term-paper', 'paper'): 255619},
      metagraph=[('author', 'paper', 'author-paper'), ('paper', 'author', 'paper-author'), ('paper', 'paper', 'paper-cite-paper'), ('paper', 'paper', 'paper-ref-paper'), ('paper', 'subject', 'paper-subject'), ('paper', 'term', 'paper-term'), ('subject', 'paper', 'subject-paper'), ('term', 'paper', 'term-paper')])
"""
            # g = g.to('cuda:0') this line will be very useful? maybe
            category = 'paper'
            num_classes = 3
