...
class BaseFlow(ABC):  # abstract base class
    ...
    def __init__(self, args):
        ...
        if hasattr(args, '_checkpoint'):
            self._checkpoint = os.path.join(args._checkpoint, f"{args.model_name}_{args.dataset_name}_{args.seed}.pt")
        else:
            if hasattr(args, 'load_from_pretrained'):
                self._checkpoint = os.path.join(args.output_dir,
                                                f"{args.model_name}_{args.dataset_name}_{args.task}_{args.seed}.pt")
            else:
                self._checkpoint = None
        # self._checkpoint = "/home/fly/xzx/HGNN_MAIN/result"
        ...
        self.logger.info("model save path is"+self._checkpoint)
        if hasattr(args, 'sperate_task'):
            print("the task are build before")
            self.logger.info("the task used the args.sperate_task")
            self.task = args.sperate_task
        else:
            print("the task build every epoch")
            self.task = build_task(args)  # it seems that we can load the data in this place
        if self.args.use_uva:
            self.hg = self.task.get_graph()
            # def get_graph(self):
            #   return self.dataset.g
        else:
            # print("the graph is set to cuda.")
            self.hg = self.task.get_graph().to("cuda:0")
    ...
    # it seems that this function is used in every dataset function.
    def preprocess(self):
        r"""
        Every trainerflow (must)should run the preprocess_feature if you want to get a feature preprocessing.
        ...
        """
        # print(self.args.activation) LeakyReLU(negative_slope=0.5)
    ...
    def feature_preprocess(self, act):
        ...
        """
        # args.hidden_dim = 8 I dont know how it set?
        if self.hg.ndata.get('h', {}) == {} or self.args.feat == 2:
            if self.hg.ndata.get('h', {}) == {}:
                ...
                self.hg.ndata.pop('h')  # drop the feature 'h'
            ...
            if self.args.task != 'node_classification':
                ...
                self.input_feature = HeteroFeature({self.category: h_dict[self.category]}, get_nodes_dict(self.hg),
