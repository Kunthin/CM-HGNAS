...
class SimpleNASController(torch.nn.Module):
    ...
    def __init__(self, args, search_space, action_list, controller_hid=40, cuda=True, mode="train",
                 softmax_temperature=5.0, tanh_c=2.5):
        if not self.check_action_list(action_list, search_space):  # check if the actionlist = searchspace
        ...
        if args and args.softmax_temperature:
            self.softmax_temperature = args.softmax_temperature  # 5.0
        else:
            self.softmax_temperature = softmax_temperature
        if args and args.tanh_c:
            self.tanh_c = args.tanh_c  # 2.5
        else:
            self.tanh_c = tanh_c
        ...
        # this step is used to collect the dimension of the search space.
        for key in self.search_space:
            self.num_tokens.append(len(self.search_space[key]))

        num_total_tokens = sum(self.num_tokens)  # count action type
        # print(num_total_tokens, controller_hid)
        # here we create the encoder with the weight and the dimension is num_total_tokens*controller_hid
        ...
        for key in self.search_space:
            ...
            decoder = torch.nn.Linear(controller_hid, size)  # 100 to the dimension of the it
    ...
    def check_action_list(action_list, search_space):
        if isinstance(search_space, dict):  # check if the search_space is dict type
    ...
    def sample(self, dataset_name,batch_size=1, with_details=False):
        torch.cuda.manual_seed(int(time.time()))
        if batch_size < 1:
            raise Exception(f'Wrong batch_size: {batch_size} < 1')
        # it seems that a batch of data are coming.
        # and also like he said ,the init input and hidden are 0
        if dataset_name in []:
            inputs = torch.ones([batch_size, self.controller_hid])
            hidden = (torch.ones([batch_size, self.controller_hid]), torch.ones([batch_size, self.controller_hid]))
        else:
        ...
        for block_idx, action_name in enumerate(self.action_list):
            ...
            # print(block_idx,entropy)
            # this action sample is with weight,the weight is the entropy
        ...
        # print("here is the actions",actions)
        dags = self._construct_action(actions)

        if with_details:
            return dags, torch.cat(log_probs), torch.cat(entropies)
        # the dags return the really value of every dimension not the index.
        return dags
