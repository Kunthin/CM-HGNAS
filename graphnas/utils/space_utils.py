...
def read_config(args):
    ...
    if args.model == 'homo_GNN':
        ...
        # here just let us to choose relation or metapath
        args.model_family = args.subgraph_extraction
    else:
        raise ValueError('Wrong model name or subgraph_extraction')
    # this line let us to find the absolute path of the file
    fileNamePath = os.path.split(os.path.realpath(__file__))[0]
    # the following if are use to find the configfile path if args.key == 'gnn_type': yamlPath = os.path.join(
    # fileNamePath, 'config/{}/{}.yaml'.format(args.configfile, args.times)) else: yamlPath = os.path.join(
    # fileNamePath, 'config/{}/{}/{}_{}.yaml'.format(args.configfile, args.key, args.gnn_type, args.times))
    yamlPath = args.yamlfile
    print(yamlPath)
    with open(yamlPath, 'r', encoding='utf-8') as f:
        cont = f.read()
        config_dict = yaml.safe_load(cont)[args.task][args.dataset]
        # this line let us to find the configure in the file
    ...
    # this for is used to set the configure
    ...
    path = './space4hgnn/config/{}/{}/{}'.format("meta_train", args.key, args.value)
