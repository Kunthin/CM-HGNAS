...
from graphnas.gnn_model_manager import CitationGNNManager, evaluate, ppi_evaluate
...
class MAMLGeoCitationManager(CitationGNNManager):
    def __init__(self, args, data):
        super(MAMLGeoCitationManager, self).__init__(args)
        ...
        self.args.in_feats = self.in_feats = self.data.x.shape[1]
        if self.args.ppi is True:
            self.args.num_class = self.n_classes = 121
        else:
    ...
    def run_model(model, optimizer, loss_fn, data, epochs, early_stop=5, tmp_model_file="geo_citation.pkl",
        ...
        for epoch in range(1, epochs + 1):
            ...
            # # compute/update  best_acc based on val_loss
            ...
            # compute/update  best_acc only based on test_acc
            if test_acc > best_performance:
        ...
        # print(f"train_acc:{model_train_acc}, val_acc:{model_val_acc}, test_acc:{best_performance}")
        if return_best:
            return model, model_train_acc, model_val_acc, best_performance
        else:
